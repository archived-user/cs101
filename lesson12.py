...
def total_enrollment(inlist):
    total_student = 0
    total_fees = 0
    for uni in inlist:
        total_student += uni[1]
        total_fees += uni[1] * uni[2]
...
def crawl_web(seed,max_depth):
    ...
    while tocrawl:
        ...
        union(crawled,[page])
        if depth < max_depth:
            for links in get_all_links(get_page(page)):
                tocrawl.append((links,depth+1))
    return crawled
"""
...
def check_col(square, c):
    col = []
    n = len(square)
    while n > 0:
        n -= 1
        col.append(square[n][c])
    return check_row(col)
               
def check_sudoku(insquare):
    for rows in insquare:
        if not check_row(rows):
            return False
    c = 0
    while c < len(insquare):
        if not check_col(insquare, c):
            return False
        c += 1
    return True
