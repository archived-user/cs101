...
def is_offered(courses, course, hexamester):
    offerings = courses_offered(courses, hexamester)
...
def when_offered(courses,course):
    ...
    for hexamester in courses:
        for offers in courses[hexamester]:
            if offers == course:
...
def involved(courses, person):
    out = {}
    for hexamester in courses:
        courses_in_hex = courses[hexamester]
        for course in courses_in_hex:
            elements_in_course = courses_in_hex[course]
            for element in elements_in_course:
                if person == elements_in_course[element]:
                    if hexamester not in out:
                        out[hexamester] = [course]
                    else:
                        out[hexamester].append(course)
    return out
